...
public class MyToolItemQuote extends ARE_ToolItem_Abstract {

    AREditText editText;

    public MyToolItemQuote(TextView textView,AREditText editText) {
    ...
    public IARE_Style getStyle() {
        if (mStyle == null) {
            mStyle = new MyQuote((TextView) mToolItemView);
            ((MyQuote)mStyle).setEditText(editText);
        }
        return mStyle;
    }
    ...
    public void onSelectionChanged(int selStart, int selEnd) {
        ...
        if (selStart > 0 && selStart == selEnd) {
            MySpan[] quoteSpans = editable.getSpans(selStart - 1, selStart, MySpan.class);
            if (quoteSpans != null && quoteSpans.length > 0) {
                quoteExists = true;
            }
        } else {
            MySpan[] quoteSpans = editable.getSpans(selStart, selEnd, MySpan.class);
            if (quoteSpans != null && quoteSpans.length > 0) {
